...
List_score_matrix=foreach(k = 1:N_image) %dopar% {
  ...
  for (i in 1:N_cluster) {
    ...
    for (j in 1:N_cluster) {
      ...
      Joint_pattern = ppp(c(X_pattern$x,Y_pattern$x),
                          c(X_pattern$y,Y_pattern$y),window = owin(X_range,yrange = Y_range))
      
      if (i!=j & npoints(X_pattern)>10 & npoints(Y_pattern)>10) {
        
        Density_X = density.ppp(X_pattern,sigma = Sigma_parameter)
        Density_X$v = Density_X$v/sum(Density_X$v)
        
        Density_Y = density.ppp(Y_pattern,sigma = Sigma_parameter)
        Density_Y$v = Density_Y$v/sum(Density_Y$v)
        
        Density_joined = density.ppp(Joint_pattern,sigma = Sigma_parameter)
        Density_joined$v = Density_joined$v/sum(Density_joined$v)
        ...
        if (Distribution_metric=="MI") {
          Distribution_distance = Density_joined$v * log(Density_joined$v/(Density_X$v*Density_Y$v))
          Distribution_distance[is.na(Distribution_distance)] =0
          Distribution_distance[is.infinite(Distribution_distance)] =0
          Distribution_distance = sum(Distribution_distance)
  ...
  rownames(Density_score_matrix) = 1:nrow(Density_score_matrix)
  colnames(Density_score_matrix) = 1:ncol(Density_score_matrix)
